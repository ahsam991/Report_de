...
--              AND c.contact_date BETWEEN '2025-01-21' AND '2025-01-23'
                -- NOTE: named parameters like custom_date do NOT work inside CROSSTAB
                --   dollar-quoted strings. To filter by a specific date, edit the date above,
                --   e.g.: AND c.contact_date = '2025-06-13'
...
--     COALESCE(q."6",q."18",q."31",q."43") AS "Consumers Fathers Name",
    ...
    -- cl.cluster_type AS "Cluster Type"  -- Uncomment together with the LEFT JOIN below when table is available
    NULL::VARCHAR AS "Cluster Type"
FROM
    qu_ans q
...
-- 	LEFT JOIN ecrm_tmp.luckies_cluster_id_list_161 cl ON cl.cluster_id = l.cluster_id
--     LEFT JOIN (
--     SELECT DISTINCT ON (cluster_name, route)
--         cluster_name,
--         route,
--         cluster_type
--     FROM ecrm_tmp.luckies_cluster_id_list_161
--     WHERE cluster_name IS NOT NULL AND route IS NOT NULL
-- 	) cl ON cl.cluster_name = l.cluster AND cl.route = l.route
   ,ecrm.user_infos ui
