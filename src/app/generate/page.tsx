...
export default function GenerateReport() {
  ...
  useEffect(() => {
    ...
    const seenAliases = new Map<string, string>(); // alias -> id

    fileRows.forEach(r => {
      const id = r[qIdx]?.trim();
      const rawAlias = r[aIdx]?.trim();
      if (!id || !rawAlias) return;
      ...
      // If alias already used by a different ID, suffix it to keep it unique
      let alias = rawAlias;
      if (seenAliases.has(alias) && seenAliases.get(alias) !== id) {
        alias = `${rawAlias} (${id})`;
      }
    ...
    if (duplicateIds.size > 0) {
      setFileError(`⚠ Duplicate Question IDs detected and skipped: ${Array.from(duplicateIds).join(", ")}`);
    } else {
      setFileError("");
    }
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div className="glass-card p-6 rounded-3xl space-y-6">
            ...
            {!file ? (
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3">
                  ...
                  <button onClick={() => { setFile(null); setQIdCol(""); setAliasCol(""); setParsedQuestions([]); setFileError(""); }} className="text-muted-foreground hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
            ...
            {fileError && <div className={`text-xs flex items-center gap-2 mt-2 ${fileError.startsWith("⚠") ? "text-amber-400" : "text-red-400"}`}><AlertTriangle className="w-4 h-4 shrink-0" />{fileError}</div>}
          </div>
      ...
      {step === 4 && (
        <div className="space-y-5">
          ...
          <div className="flex justify-between">
            ...
            <button onClick={() => { setFile(null); setParsedQuestions([]); setCampaignId(""); setFileError(""); setStep(1); }} className="inline-flex items-center gap-2 bg-primary hover:bg-primary/95 text-white font-bold text-sm px-6 py-3.5 rounded-2xl transition-all hover:scale-[1.02] shadow-lg shadow-primary/20">
